...
private:
    ...
    // Publish drive data
    ros::Publisher drive_pub, viz_pub;
...
public:
    Ransac() {
        ...
        viz_pub = n.advertise<visualization_msgs::Marker>("ransac_vis", 10);
    }
    ...
    void scan_callback(const sensor_msgs::LaserScan & lc_msg) {
        ...
        std::vector<double> ranges = lc_msg.ranges;
        Point p;
    ...
    std::vector <std::pair<Point, Point>> ransac_cal(vector <Point> points) {

        int points_size = points.size();
        int iterations = (int) (*0.1);
        std::pair <Point, Point> best_candidate_pair;
        std::vector <std::pair<Point, Point>> best_candidates;
        double best_candidate_inliers = 0;
    ...
    double calculate_dist(Point point_a, Point point_b, Point point_c) {
        double a, b, c;
        getLine (point_a, point_b, a, b, c);
        return abs (a * point_c.x + b * point_c.y + c) / sqrt (a * a + b * b);
    }
